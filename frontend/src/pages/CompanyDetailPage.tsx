...
export const CompanyDetailPage: React.FC = () => {
  ...
  const handleDeleteDeployment = async (depId: string, locId: string) => {
    if (!window.confirm('Are you sure you want to delete this deployment record?')) return;
    try {
      await deleteDeployment(depId);
      setLocationDeployments(prev => ({
        ...prev,
        [locId]: (prev[locId] || []).filter(d => d.id !== depId)
      }));
    } catch {
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 sm:p-7 shadow-xs space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="flex items-start space-x-4">
            <div className="w-14 h-14 rounded-xl bg-blue-600 text-white flex items-center justify-center font-bold text-2xl shadow-md shadow-blue-600/20 shrink-0">
          ...
          <div className="flex items-center space-x-3 shrink-0">
            ...
            <div className="bg-blue-50 px-4 py-2.5 rounded-xl border border-blue-200 text-center">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Each datacenter facility or office can host multiple deployments across different dates and product lines.
            </p>
          </div>
            ...
            {locations.map((loc) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Deployments List under this Location */}
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center space-x-2">
                        <Server className="w-4 h-4 text-slate-400" />
                        <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                          Deployment Records ({deps.length})
                        </h4>
                      </div>
                      <span className="text-[11px] text-slate-400">
                        Historical timeline throughout multiple dates
                      </span>
                    </div>
                                  ...
                                  <td className="py-3 px-3 text-right whitespace-nowrap">
                                    <div className="flex items-center justify-end space-x-2">
                                      <Link
                                        ...
                                        className="inline-flex items-center space-x-1 text-xs font-semibold text-blue-600 hover:text-blue-800 px-2 py-1 rounded hover:bg-blue-50 transition-colors"
                                      >
                                        <span>Details</span>
                                      ...
                                      <button
                                        type="button"
                                        onClick={() => handleDeleteDeployment(dep.id!, loc.id)}
